...
class LocalPlanner:
    ...
    def get_goal_state_set(self, goal_index, goal_state, waypoints, ego_state):
        ...
        # ------------------------------------------------------------------
        if goal_index >= len(waypoints[0])-1:
        ...
        # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
        goal_x = np.cos(-ego_state[2])*goal_state_local[0] - \
            np.sin(-ego_state[2])*goal_state_local[1]
        goal_y = np.sin(-ego_state[2])*goal_state_local[0] + \
            np.cos(-ego_state[2])*goal_state_local[1]
        # ------------------------------------------------------------------
