    ...
    def collision_check(self, paths, obstacles):
        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                # --------------------------------------------------------------
                circle_locations[:, 0] = np.array(path[0][j]) + \
                    np.array(self._circle_offsets)*np.cos(path[2][j])
                circle_locations[:, 1] = np.array(path[1][j]) + \
                    np.array(self._circle_offsets)*np.sin(path[2][j])
                # --------------------------------------------------------------
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                score = np.linalg.norm(
                    [paths[i][0][-1]-goal_state[0], paths[i][1][-1]-goal_state[1]], ord=2)
                # --------------------------------------------------------------
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            ...
                            score += self._weight * \
                                np.linalg.norm(
                                    [np.subtract(paths[i][0], paths[j][0]), np.subtract(paths[i][1], paths[j][1])])
                            # --------------------------------------------------
